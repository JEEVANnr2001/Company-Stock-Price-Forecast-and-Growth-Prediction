...
def app():
    st.title('Total Liabilites Forecast')
    #!/usr/bin/env python
    # coding: utf-8
...
#     st.pyplot(fig)
    ...
    data_training = df['Liabilities'].values.reshape(-1,1)
    data_testing = df['Liabilities'].values.reshape(-1,1)
    x_t=df[['Year','Quarter']]
...
#     print(x_t)
    ...
    st.subheader('Forcasted Liabilities')
...
#     st.pyplot(fig2)
    ...
    plt.plot(df['Date'],pred)
    plt.plot(d,pred2)
    plt.legend(['Total Liabilities', 'Predicted Liabilities', 'Forecasted Liabilities'])
    plt.xlabel('Time')
    plt.ylabel('total liabilities(in 100 billions)')
    st.pyplot(fig4)
